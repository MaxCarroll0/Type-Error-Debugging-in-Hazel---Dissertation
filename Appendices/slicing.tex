...
\begin{definition}[Dynamic Synthesising Function Analysis Slice]
...
\begin{figure}[H]
\small
\fbox{$\elaborationSynthesisSlice{e}{\tau}{\S}{d}{\Delta}$}\ \ \ $e$ synthesises type $\tau$ with synthesis slice $\S$ and elaborates to $d$ and hole context $\Delta$.
\tiny
...
\small
\fbox{$\elaborationAnalysisSlice{\C^\F}{e}{\tau}{d}{\tau'}{\S' \mid \A'}{\Delta}$}\ \ \ $e$ in minimally scoped checking context $\C^\F$ analyses against type $\tau$ with analysis slice $\A$ and elaborates to $d$ of consistent type $\tau'$ which has synthesis slice part $\S'$ and analysis slice part $\A'$ (structurally matching the synthetic part).
\tiny
...
\begin{figure}[H]
\fbox{$S \groundmatch \S'$}\ \ \ $\S$ matches ground slices $\S'$
