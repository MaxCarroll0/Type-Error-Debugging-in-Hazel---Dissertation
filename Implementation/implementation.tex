...
For a derivation $\synthesis{e}{\tau}$, containing a sub-derivation $\analysis[\Gamma']{e'}{\tau'}$ with checking context $\C$. Then we have that $\synthesis[\Gamma]{\C\{e'\}}{\tau''}$. An \textit{analysis slice} of $e'$ is a program context slice $\Cs^\gamma$ such that:
\begin{itemize}
\item $\Cs$ is a slice of $\C$, that is, $\Cs \sqsubseteq \C$.
\item $e$ still synthesises a type and contains a sub-derivation checking $e'$ against $\tau'$ in checking context $\Cs$.
\item $\gamma$ contains the required assumptions to type the context with a type consistent\footnote{$e'$ within the sliced context might now synthesise a \textit{less precise} type.} with $\tau''$: $\analysis[\gamma]{\Cs\{\gap\}}{\tau''}$.
\end{itemize}
\end{definition}
