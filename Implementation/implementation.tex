...
The Hazel architecture demonstrates in \cref{fig:Architecture} with green files/directories added, orange files modified \textit{significantly}, and blue files modified \textit{insignificantly}\footnote{However, these refactorings still took a significant amount of time, sometimes requiring careful management of slices.} or unmodified. 97 files were added/edited, not all can fit, so anything omitted is considered insignificant to understanding this project.
...
\end{figure} 
 ...
 & & & {\color{Green}\ \ \ \  \ \ \ \ $\vdash$} & {\color{Green}\texttt{./Searching.re}} & Default search procedures (to values, full traces, to cast errors, etc.)\\
